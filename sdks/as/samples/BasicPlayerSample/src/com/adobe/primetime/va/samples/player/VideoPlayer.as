...
package com.adobe.primetime.va.samples.player {
    ...
    public class VideoPlayer extends FLVPlayback {
        ...
        private function _onStateChange(event:VideoEvent):void {
            if (_buffering) {
                _buffering = false;
                CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.BUFFER_COMPLETE));
            }

            if (_seeking) {
                _seeking = false;
                CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.SEEK_COMPLETE));
            }
            ...
            switch (event.state) {
                ...
                case VideoState.PLAYING:
                    _openVideoIfNecessary();
                    CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.PLAY));
                    break;
                case VideoState.BUFFERING:
                    _buffering = true;
                    CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.BUFFER_START));
                    break;
                case VideoState.SEEKING:
                    _openVideoIfNecessary();
                    _seeking = true;
                    CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.SEEK_START));
                    break;
                case VideoState.PAUSED:
                    _openVideoIfNecessary();
                    CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.PAUSE));
                    break;
            }
        }
        ...
        private function _closeVideo():void {
            if (_videoLoaded) {
                ...
                CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.COMPLETE));

                CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.VIDEO_UNLOAD));
        ...
        private function _startVideo():void {
            ...
            CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.VIDEO_LOAD));
        }
        ...
        private function _startChapter1():void {
            ...
            CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.CHAPTER_START));
        }
        ...
        private function _completeChapter():void {
            ...
            CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.CHAPTER_COMPLETE));
        }
        ...
        private function _startAd():void {
            ...
            // Start the ad.
            CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.AD_START));
        }

        private function _completeAd():void {
            // Complete the ad.
            CommCenter.sharedChannel.trigger(new PlayerEvent(PlayerEvent.AD_COMPLETE));
